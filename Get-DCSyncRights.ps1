...
#>
    ...
    ForEach ($Domain in $Domains) {
        ...
        $DSReplicationGetChanges = Get-Acl -Path $NetBIOSName":\$DomainDN" | Select -ExpandProperty Access | Where {$_.ObjectType -eq "1131f6aa-9c07-11d1-f79f-00c04fc2dcd2" -and $_.AccessControlType -eq "Allow"}
        $DSReplicationGetChangesALL = Get-Acl -Path $NetBIOSName":\$DomainDN" | Select -ExpandProperty Access | Where {$_.ObjectType -eq "1131f6ad-9c07-11d1-f79f-00c04fc2dcd2" -and $_.AccessControlType -eq "Allow"}
